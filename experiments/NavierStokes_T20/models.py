...
class DyMemCell(nn.Module):
    ...
    def forward(self, x, h, c):
        ...
        gates = self.conv3d(torch.cat([combined, combined], dim=1)).squeeze(2)
...
class NeuroSD(nn.Module):
    def __init__(self, in_channels=1, hid_channels=128, out_frames=10):
        super().__init__()
        self.hid_channels, self.out_frames = hid_channels, out_frames
    ...
    def forward(self, x):
        ...
        h1 = torch.zeros(b, self.hid_channels, 32, 32).to(x.device)
        c1 = torch.zeros(b, self.hid_channels, 32, 32).to(x.device)
        h2 = torch.zeros(b, self.hid_channels, 32, 32).to(x.device)
        c2 = torch.zeros(b, self.hid_channels, 32, 32).to(x.device)

        for i in range(t):
            f_t = self.encoder(x[:, i])

            # SDO Block with residual connection
