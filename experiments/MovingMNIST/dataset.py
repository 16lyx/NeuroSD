...
def load_cifar(root, data_name='mnist_cifar'):
    ...
    if 'cifar' in data_name:
        ...
        data = np.concatenate(
            [cifar_train.data, cifar_test.data],
            axis=0
        ).reshape(-1, 32, 32, 3)
    return data
...
def load_fixed_set(root, data_name='mnist'):
    # Directly load the fixed Moving MNIST test sequence file.
    path = os.path.join(root, 'mnist_test_seq.npy')

    if not os.path.exists(path):
        raise FileNotFoundError(
            f"Dataset file not found at {path}. "
            f"Please make sure the file is placed under the data directory."
        )

    # Load data with shape [20, 10000, 64, 64].
    dataset = np.load(path)

    # Keep the dimension format consistent as [T, N, H, W, C].
    if dataset.ndim == 3:
...
class MovingMNIST(Dataset):
    ...
    def __init__(
        ...
        # Always load the fixed .npy dataset directly.
    ...
    def get_random_trajectory(self, seq_length):
        """Generate a random trajectory for a MNIST digit."""
        ...
        for i, v_x, v_y in zip(range(seq_length), v_xs, v_ys):
            # Take a step along the velocity direction.
        ...
        # Scale the trajectory coordinates to the canvas size.
    ...
    def generate_moving_mnist(self, num_digits=2, background=False):
        """Generate a Moving MNIST video with random digit trajectories."""
        if not background:
            data = np.zeros(
        ...
        for n in range(num_digits):
            ...
            for i in range(self.n_frames_total):
                ...
                if not background:
                    ...
                    data[i, top:bottom, left:right, ...] = np.maximum(
                        data[i, top:bottom, left:right, ...],
                        np.repeat(digit_image[..., np.newaxis], 3, axis=2)
                    )
    ...
    def _augment_seq(self, imgs, crop_scale=0.94):
        """Apply data augmentation to a video sequence."""
        _, _, h, w = imgs.shape

        imgs = F.interpolate(
            imgs,
            scale_factor=1 / crop_scale,
            mode='bilinear'
        )
    ...
    def __getitem__(self, idx):
        ...
        input_seq = images[:self.n_frames_input]

        if self.n_frames_output > 0:
            output_seq = images[self.n_frames_input:length]
        else:
            output_seq = []
        ...
        if self.use_augment:
            imgs = self._augment_seq(
                torch.cat([input_seq, output_seq], dim=0),
                crop_scale=0.94
            )
            input_seq = imgs[:self.n_frames_input, ...]
            output_seq = imgs[
